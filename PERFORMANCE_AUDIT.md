...
## H1 — Every request writes two near-identical log rows
...
**Recommendation.** Keep one table. `RequestLog` is a superset of `ApiLog`, so
drop the `ApiLog` write and point the Logs UI's "api" view at `RequestLog`, or
merge them. This halves per-request writes. Not applied here because it changes
the Logs screen and needs a migration.
...
## H3 — Institute list is N+1 across several tables
...
**Recommendation.** Batch the sub-lookups: fetch subscriptions, branding, admins
and module counts for all institutes on screen in one query each, keyed by
institute id, then assemble. This turns ~5M queries into ~5. Not applied here
because it is a sizeable rewrite of a much-used serializer and wants a live test.
...
1. **H2** — done, biggest per-view query win on a screen instructors hit often.
2. **M1** — done, removes a whole-document scan from normal navigation.
3. **H1** — drop the duplicate request-log write (needs a Logs-UI decision).
4. **H3** — batch the institute serializer (needs a live test).
