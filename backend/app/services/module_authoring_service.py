...
def create_module(db: Session, actor: User, data: dict, ip: Optional[str]) -> dict:
    blueprint = get_blueprint(data["module_type"])
    module = ExamModule(
        **data,
    ...
    _audit(db, actor, "exam_module.create", module.id, ip, {"module_type": module.module_type})
    db.commit()
...
def delete_module(db: Session, actor: User, module_id: int, ip: Optional[str]) -> None:
    ...
    if module.status == "published":
        raise HTTPException(status_code=400, detail="Published modules must be archived or returned to draft before deletion")
    paths = [settings.storage_path / asset.file_path for asset in module.assets]
    _audit(db, actor, "exam_module.delete", module.id, ip, {"title": module.title})
    db.delete(module)
    db.commit()
    for path in paths:
        path.unlink(missing_ok=True)
