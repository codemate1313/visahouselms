...
@app.get("/health/attempt-debug")
def debug_attempt():
    from app.database import SessionLocal
    from app.models.attempt import TestAttempt
    db = SessionLocal()
    try:
        attempt = db.query(TestAttempt).filter(TestAttempt.id == 137).first()
        if not attempt:
            return {"error": "Attempt 137 not found"}
        other_attempts = db.query(TestAttempt).filter(TestAttempt.user_id == attempt.user_id).all()
        return {
            "attempt_137": {
                "id": attempt.id,
                ...
                "part_grades_count": len(attempt.part_grades),
            },
            "other_attempts": [
                {
                    "id": a.id,
                    ...
                    "status": a.status,
                    "phase": (a.content_snapshot or {}).get("phase"),
                }
                for a in other_attempts
            ]
        }
