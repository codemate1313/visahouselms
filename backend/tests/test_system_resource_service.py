...
class SystemResourceServiceTestCase(unittest.TestCase):
    ...
    def test_memory_response_exposes_server_labels(self):
        ...
        self.assertIn("approximate", reading)
        self.assertIn("total_bytes", reading)
        self.assertIn("used_bytes", reading)
        self.assertIn("available_bytes", reading)
        self.assertIn("cached_bytes", reading)
        self.assertIn("used_percent", reading)
        self.assertIn("swap", reading)
        self.assertIn("app", reading)
        if reading.get("app"):
