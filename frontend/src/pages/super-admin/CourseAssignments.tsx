...
export function CourseAssignments() {
  ...
  return <div><div className="page-header"><div><h1>{course.title}</h1><p className="page-subtitle">Control website visibility, lifecycle, and institute access.</p></div><Link to="/super-admin/courses">Back to hierarchy</Link></div>
    ...
    <div className="assignment-grid"><section className="workspace-panel"><h2>Assign to institute</h2>{course.status !== "published" ? <div className="banner warning">Publish the course before assigning it.</div> : <form onSubmit={assign}><label htmlFor="institute">Institute</label><SearchableSelect options={available.map((inst) => ({ value: inst.id, label: `${inst.name} (${inst.subscription_state})` }))} value={selected} onChange={(val) => setSelected(String(val))} placeholder="Select an active institute..." searchPlaceholder="Type to search institute..." disabled={saving} /><button className="top-gap" disabled={saving || !selected}>{saving ? "Assigning..." : "Grant access"}</button></form>}</section><section className="workspace-panel"><h2>Course resources</h2>{course.assets.length ? <ul className="resource-links">{course.assets.map((asset) => <li key={asset.id}><span className={`asset-icon ${asset.asset_type}`}>{asset.asset_type.toUpperCase()}</span><a href={`${API_BASE_URL}${asset.file_url}`} target="_blank" rel="noreferrer">{asset.title}</a></li>)}</ul> : <p className="empty-message">No resources uploaded.</p>}</section></div>
    <h2 className="section-title">Institute access history</h2><table className="data-table"><thead><tr><th>Institute</th><th>Assigned</th><th>Status</th><th className="table-actions-heading">Actions</th></tr></thead><tbody>{!course.assignments?.length ? <tr><td colSpan={4} className="empty-cell">Not assigned to any institute.</td></tr> : course.assignments.map((item) => <tr key={item.id}><td>{item.institute_name}</td><td>{new Date(item.assigned_at).toLocaleDateString()}</td><td><span className={`badge ${item.is_active ? "badge-green" : "badge-gray"}`}>{item.is_active ? "Active" : "Revoked"}</span></td><td className="table-actions">{item.is_active && <button className="danger" onClick={() => unassign(item.institute_id)} aria-label="Revoke access" data-tooltip="Revoke access"><Icon name="revoke" /></button>}</td></tr>)}</tbody></table>
  </div>;
}
