...
export function Terminal() {
  ...
  useEffect(() => {
    if (connected && containerRef.current && !termRef.current) {
      ...
      term.writeln(`\x1b[1;32m${strings.banner.welcome}\x1b[0m`);
      term.writeln(`${strings.banner.selectCommand}\r\n`);
  ...
  async function connect(event: FormEvent) {
    ...
    try {
      ...
      socket.onclose = () => {
        setConnected(false);
        setRunning(false);
        termRef.current?.writeln(`\x1b[1;33m${strings.connectionClosed}\x1b[0m`);
      };
      socketRef.current = socket;
    } catch (err: unknown) {
      setError(extractErrorMessage(err, strings.errors.open));
    } finally {
      setConnecting(false);
    }
  }
  ...
  if (!connected) {
    return <TerminalAuthGate password={password} onPasswordChange={setPassword} error={error} connecting={connecting} onConnect={connect} />;
  }

  return <TerminalView presets={presets} running={running} onRunPreset={runPreset} containerRef={containerRef} />;
}
