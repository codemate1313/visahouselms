...
export function CourseCatalog() {
  ...
  return <div><div className="page-header"><div><h1>Instructor Course Hierarchy</h1><p className="page-subtitle">Inspect every SA Instructor, their courses, contents, publication dates, and distribution.</p></div></div>
    <form className="filter-bar" onSubmit={submit}><input placeholder="Search course title..." value={search} onChange={(event) => setSearch(event.target.value)} /><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></select><button>Search</button></form>
