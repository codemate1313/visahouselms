...
export function TestEditor() {
  ...
  async function deleteTest() { if (!test || !window.confirm(`Delete “${test.title}”?`)) return; try { await apiClient.delete(`/instructor/authoring/tests/${test.id}`); navigate("/instructor/tests"); } catch (err: unknown) { setError(extractErrorMessage(err, "Failed to delete the test.")); } }
  ...
  return <div>
    <div className="page-header"><div><h1>{isNew ? "New Test" : test?.title}</h1>{test && <p className="page-subtitle">{TYPE_LABELS[test.assessment_type]} · {test.course_title}</p>}</div><Link className="text-link" to="/instructor/tests">← All tests</Link></div>
    ...
    <form className="form-card wide test-details-form" onSubmit={saveDetails}><h2>Test details</h2><div className="form-grid"><div><label htmlFor="test-course">Course</label><select id="test-course" value={form.course_id} disabled={!canEdit} onChange={(event) => { const course_id = event.target.value; setForm({ ...form, course_id }); setQuestionIds([]); loadQuestions(course_id); }} required>{courses.map((course) => <option value={course.id} key={course.id}>{course.title}</option>)}</select></div><div><label htmlFor="test-type">Test type</label><select id="test-type" value={form.assessment_type} disabled={!canEdit} onChange={(event) => setForm({ ...form, assessment_type: event.target.value })}>{Object.entries(TYPE_LABELS).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></div></div><label htmlFor="test-title">Title</label><input id="test-title" value={form.title} readOnly={!canEdit} onChange={(event) => setForm({ ...form, title: event.target.value })} required /><label htmlFor="test-description">Description</label><textarea id="test-description" rows={3} value={form.description} readOnly={!canEdit} onChange={(event) => setForm({ ...form, description: event.target.value })} /><label htmlFor="test-instructions">Student instructions</label><textarea id="test-instructions" rows={4} value={form.instructions} readOnly={!canEdit} onChange={(event) => setForm({ ...form, instructions: event.target.value })} /><label htmlFor="test-duration">Time limit in minutes</label><input id="test-duration" type="number" min="1" max="600" value={form.duration_minutes} readOnly={!canEdit} onChange={(event) => setForm({ ...form, duration_minutes: event.target.value })} placeholder="Leave empty for untimed" />{canEdit && <div className="form-actions"><button type="submit" disabled={saving || !form.course_id}>{saving ? "Saving..." : isNew ? "Create Test" : "Save Details"}</button>{test && <button type="button" className="danger-text" onClick={deleteTest}>Delete test</button>}</div>}</form>
    {test && <div className="test-builder-grid"><section className="authoring-panel"><div className="section-heading"><div><h2>Course questions</h2><p>Select questions to add to this test.</p></div><span className="count-chip">{filtered.length} shown</span></div><div className="question-picker-filters"><input placeholder="Search questions..." value={search} onChange={(event) => setSearch(event.target.value)} /><select value={section} onChange={(event) => setSection(event.target.value)}><option value="">All sections</option><option value="listening">Listening</option><option value="reading">Reading</option><option value="writing">Writing</option><option value="speaking">Speaking</option></select></div>{!available.length ? <div className="empty-state compact-empty"><h2>No questions available</h2><p>Create a question bank for {test.course_title} first.</p><Link className="button-link" to="/instructor/question-banks/new">New Question Bank</Link></div> : <div className="question-picker-list">{filtered.map((question) => <label className={`question-picker-item${questionIds.includes(question.id) ? " chosen" : ""}`} key={question.id}><input type="checkbox" disabled={!canEdit} checked={questionIds.includes(question.id)} onChange={() => toggleQuestion(question.id)} /><span><strong>{question.prompt}</strong><small>{question.section} · {question.bank_title} · {question.points} pt</small></span></label>)}</div>}</section>
      <section className="authoring-panel selected-questions-panel"><div className="section-heading"><div><h2>Test order</h2><p>Students receive questions in this order.</p></div><span className="count-chip">{questionIds.length}</span></div>{!questionIds.length ? <p className="empty-message">Select questions from the course bank.</p> : <ol className="test-order-list">{questionIds.map((questionId, index) => { const question = byId.get(questionId) ?? test.questions?.find((item) => item.id === questionId); return <li key={questionId}><div><strong>{question?.prompt ?? `Question ${questionId}`}</strong><small>{question?.section} · {question?.bank_title}</small></div>{canEdit && <div><button aria-label="Move up" disabled={index === 0} onClick={() => moveQuestion(index, -1)}>↑</button><button aria-label="Move down" disabled={index === questionIds.length - 1} onClick={() => moveQuestion(index, 1)}>↓</button><button aria-label="Remove" className="danger-text" onClick={() => toggleQuestion(questionId)}>×</button></div>}</li>; })}</ol>}{canEdit && <button className="save-order-button" onClick={saveQuestions} disabled={saving}>{saving ? "Saving..." : "Save Question Order"}</button>}</section>
    </div>}
  </div>;
}
