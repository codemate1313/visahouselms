...
export function Tests() {
  ...
  return <div>
    ...
    <form className="filter-bar responsive-filters" onSubmit={submit}><input placeholder="Search tests..." value={search} onChange={(event) => setSearch(event.target.value)} /><SearchableSelect ariaLabel="Course" options={[{ value: "", label: "All courses" }, ...courses.map((course) => ({ value: course.id, label: course.title }))]} value={courseId} onChange={(value) => setCourseId(String(value))} searchPlaceholder="Search courses..." className="status-filter-select" /><SearchableSelect ariaLabel="Status" options={[{ value: "", label: "All statuses" }, { value: "draft", label: "Draft" }, { value: "published", label: "Published" }, { value: "archived", label: "Archived" }]} value={status} onChange={(value) => setStatus(String(value))} searchable={false} className="status-filter-select" /><label className="inline-check"><input type="checkbox" checked={mine} onChange={(event) => setMine(event.target.checked)} /> My tests</label><button type="submit">Search</button></form>
