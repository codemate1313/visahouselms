...
export function Tests() {
  ...
  return <div>
    <div className="page-header"><div><h1>Test Builder</h1><p className="page-subtitle">Assemble course question banks into timed IELTS assessments.</p></div><Link className="button-link" to="/instructor/tests/new">+ New Test</Link></div>
    ...
    {loading ? <p>Loading...</p> : !tests.length ? <div className="empty-state"><h2>No tests found</h2><p>Create a test and add questions from one of its course banks.</p><Link className="button-link" to="/instructor/tests/new">Create Test</Link></div> : <div className="test-card-grid">{tests.map((test) => <Link className="test-card" to={`/instructor/tests/${test.id}`} key={test.id}><div className="test-card-head"><span className={`badge ${STATUS_CLASS[test.status]}`}>{test.status}</span><span>{TYPE_LABELS[test.assessment_type]}</span></div><h2>{test.title}</h2><p>{test.description || "No description added."}</p><dl><div><dt>Questions</dt><dd>{test.question_count}</dd></div><div><dt>Points</dt><dd>{test.total_points}</dd></div><div><dt>Time</dt><dd>{test.duration_minutes ? `${test.duration_minutes} min` : "Untimed"}</dd></div></dl><small>{test.course_title} · By {test.created_by_name}</small></Link>)}</div>}
  </div>;
}
