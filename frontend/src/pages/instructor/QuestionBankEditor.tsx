...
export function QuestionBankEditor() {
  ...
  return <div>
    ...
    <form className="form-card wide bank-details-form" onSubmit={saveBank}>
      <h2>Bank details</h2>
      <div className="form-grid"><div><label htmlFor="bank-course">Course</label><select id="bank-course" value={bankForm.course_id} onChange={(event) => setBankForm({ ...bankForm, course_id: event.target.value })} required disabled={!canEdit}>{courses.map((course) => <option value={course.id} key={course.id}>{course.title}</option>)}</select></div><div><label htmlFor="bank-section">IELTS section</label><select id="bank-section" value={bankForm.section} onChange={(event) => setBankForm({ ...bankForm, section: event.target.value })} disabled={!canEdit}><option value="listening">Listening</option><option value="reading">Reading</option><option value="writing">Writing</option><option value="speaking">Speaking</option></select></div></div>
    ...
    {preview && <section className="import-review"><div className="import-review-header"><div><h2>Review extracted content</h2><p>{preview.question_count} detected · {preview.warning_count} warnings · {preview.source_filename}</p></div><div className="review-actions"><button className="secondary-button" onClick={() => setPreview(null)}>Discard</button><button onClick={commitImport} disabled={importing || selectedImports.size === 0}>{importing ? "Importing..." : `Import ${selectedImports.size} Selected`}</button></div></div>
      ...
      <div className="preview-list">{preview.questions.map((question, index) => <article className={`preview-question${selectedImports.has(index) ? " selected" : ""}`} key={index}>
        ...
        <div className="form-grid"><div><label>Type</label><select value={question.question_type} onChange={(event) => updatePreviewQuestion(index, { question_type: event.target.value as QuestionType })}>{QUESTION_TYPES.map((type) => <option value={type.value} key={type.value}>{type.label}</option>)}</select></div><div><label>Correct answer key(s)</label><input value={question.correct_answers.join(", ")} onChange={(event) => updatePreviewQuestion(index, { correct_answers: event.target.value.split(",").map((answer) => answer.trim().toUpperCase()).filter(Boolean) })} placeholder="B or A, C" /></div></div>
...
function QuestionForm({ question, onChange, onTypeChange, onOptionChange, onAddOption, onRemoveOption, onToggleCorrect, onSubmit, onCancel, saving, editing }: QuestionFormProps) {
  ...
  return <form className="question-form" onSubmit={onSubmit}>
    <div className="form-grid"><div><label htmlFor="question-type">Question type</label><select id="question-type" value={question.question_type} onChange={(event) => onTypeChange(event.target.value as QuestionType)}>{QUESTION_TYPES.map((type) => <option value={type.value} key={type.value}>{type.label}</option>)}</select></div><div><label htmlFor="question-difficulty">Difficulty</label><select id="question-difficulty" value={question.difficulty} onChange={(event) => onChange({ ...question, difficulty: event.target.value as QuestionDraft["difficulty"] })}><option value="easy">Easy</option><option value="medium">Medium</option><option value="hard">Hard</option></select></div></div>
